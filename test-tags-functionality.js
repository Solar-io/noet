...
async function testTagsFunctionality() {
  ...
  try {
    ...
    if (notesWithTags.length > 0) {
      ...
      console.log(`   Example: "${firstTaggedNote.title}" has tags: ${JSON.stringify(firstTaggedNote.tags)}`);
    }
    ...
    if (fullNote.tags && fullNote.tags.length > 0) {
      ...
      console.log(`✅ Tag removed. Remaining tags: ${JSON.stringify(updatedNote.tags)}`);
    }
    ...
    if (tags.length === 0) {
      console.log('   (This is expected - the backend tags system is separate from note string tags)');
    }
