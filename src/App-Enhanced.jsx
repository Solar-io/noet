...
import { createEditor, Transforms, Editor, Text, Element as SlateElement, Node } from 'slate';
...
import { 
  ...
  ChevronRight, Edit2, Type, Palette, AlignLeft, AlignCenter, AlignRight,
  Quote, Code, Heading1, Heading2, Heading3, Link, Highlighter, Archive,
  Star, Calendar, Layers, Menu, SearchX, FolderPlus, TagIcon, MousePointer,
  Home, Inbox, Archive as ArchiveIcon, Trash, Notebook, Bell
} from 'lucide-react';
...
const initialValue = [
  {
    type: 'paragraph',
    children: [{ text: 'Welcome to your new note! Try the formatting options in the toolbar above.' }],
  },
];
...
const Sidebar = ({ 
  ...
  const sidebarItems = [
    { id: 'inbox', label: 'All Notes', icon: <Home size={18} />, count: notes?.length || 0 },
    { id: 'starred', label: 'Starred', icon: <Star size={18} />, count: notes?.filter(n => n.starred)?.length || 0 },
    { id: 'recent', label: 'Recent', icon: <Clock size={18} />, count: notes?.filter(n => {
      const dayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
      return new Date(n.updated) > dayAgo;
    })?.length || 0 },
    { id: 'trash', label: 'Trash', icon: <Trash size={18} />, count: trashedNotes?.length || 0 },
  ];
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-4 py-2">
          <div className="flex items-center justify-between mb-2">
            ...
            >
              <ChevronRight size={14} className={`transform transition-transform ${showNotebooks ? 'rotate-90' : ''}`} />
              <span>Notebooks</span>
            </button>
            <button
        ...
        <div className="px-4 py-2">
          <div className="flex items-center justify-between mb-2">
            <button
              ...
              className="flex items-center space-x-2 text-sm font-medium text-gray-700 hover:text-gray-900"
            >
              <ChevronRight size={14} className={`transform transition-transform ${showTags ? 'rotate-90' : ''}`} />
              <span>Tags</span>
            </button>
            <button
              onClick={() => {}}
            ...
            {filteredNotes.map(note => (
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  ...
                  {note.starred && <Star size={14} className="text-yellow-500 ml-2" />}
                </div>
...
const NoteEditor = ({ 
  ...
  const renderElement = useCallback(({ attributes, children, element }) => {
    ...
    switch (element.type) {
      ...
      case 'heading-three':
        return (
          <h3 style={style} {...attributes} className="text-xl font-medium my-2">
            {children}
          </h3>
        );
      case 'list-item':
      ...
      notes: [
        {
          ...
          content: JSON.stringify([
            ...
            {
              type: 'paragraph',
              children: [{ text: 'This is your demo account. Try out all the features:' }],
            },
            {
              type: 'bulleted-list',
              children: [
                { type: 'list-item', children: [{ text: 'Rich text formatting with ' }, { text: 'bold', bold: true }, { text: ', ' }, { text: 'italic', italic: true }, { text: ', and ' }, { text: 'underline', underline: true }] },
                { type: 'list-item', children: [{ text: 'Create lists, headings, and quotes' }] },
                { type: 'list-item', children: [{ text: 'Organize notes with tags and notebooks' }] },
                { type: 'list-item', children: [{ text: 'Search and filter your notes' }] },
              ],
            },
            {
              type: 'paragraph',
        ...
        },
        {
          ...
          content: JSON.stringify([
            ...
            },
            {
              type: 'numbered-list',
              children: [
                { type: 'list-item', children: [{ text: 'Use Ctrl+B for bold, Ctrl+I for italic' }] },
                { type: 'list-item', children: [{ text: 'Try the toolbar for more formatting options' }] },
                { type: 'list-item', children: [{ text: 'Notes auto-save as you type' }] },
                { type: 'list-item', children: [{ text: 'Use tags to organize your content' }] },
              ],
            },
          ]),
          tags: ['tips', 'help'],
