  ...
  try {
    ...
    for (const notebook of notebooks) {
      ...
      if (response.ok) {
        ...
        console.log(
          `✅ Created notebook: ${notebook.name} (${created.id.substring(
            0,
            8
          )}...)`
        );
    ...
    for (const tag of tags) {
      ...
      if (response.ok) {
        ...
        console.log(
          `✅ Created tag: ${tag.name} (${created.id.substring(0, 8)}...)`
        );
      } else {
    ...
    for (const folder of folders) {
      ...
      if (response.ok) {
        ...
        console.log(
          `✅ Created folder: ${folder.name} (${created.id.substring(0, 8)}...)`
        );
      } else {
    ...
    console.log(
      `📊 Created: ${createdNotebooks.length} notebooks, ${createdTags.length} tags, ${createdFolders.length} folders`
    );
