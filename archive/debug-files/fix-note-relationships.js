...
async function fixNoteRelationships() {
  try {
    ...
    console.log(
      `📊 Found: ${notebooks.length} notebooks, ${tags.length} tags, ${notes.length} notes`
    );

    // Get the first few entities to use
    const workNotebook = notebooks.find((nb) => nb.name.includes("Work"));
    const personalNotebook = notebooks.find((nb) =>
      nb.name.includes("Personal")
    );
    const learningNotebook = notebooks.find((nb) =>
      nb.name.includes("Learning")
    );
    ...
    for (let i = 0; i < Math.min(10, notes.length); i++) {
      ...
      if (Object.keys(updates).length > 0) {
        const updateResponse = await fetch(
          `${BASE_URL}/api/${USER_ID}/notes/${note.id}`,
          {
            method: "PUT",
        ...
        if (updateResponse.ok) {
          const notebookName = updates.notebook
            ? notebooks.find((nb) => nb.id === updates.notebook)?.name ||
              "Unknown"
            : "None";
          const tagNames = updates.tags
            ? updates.tags
                .map(
                  (tagId) => tags.find((t) => t.id === tagId)?.name || "Unknown"
                )
                .join(", ")
            : "None";

          console.log(
            `✅ Updated "${note.title.substring(
              0,
              30
            )}..." → Notebook: ${notebookName}, Tags: [${tagNames}]`
          );
        } else {
    ...
    console.log(
      "Now test the UI to see actual notebook names and tag filters."
    );
  } catch (error) {
