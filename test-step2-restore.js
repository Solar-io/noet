...
async function testStep2RestoreAndDelete() {
  console.log(
    "🔄 Testing Step 2: Restore and Permanent Delete functionality..."
  );
  ...
  try {
    ...
    if (!softDeleteResponse.ok) {
      throw new Error(
        `Failed to soft delete note: ${softDeleteResponse.status}`
      );
    }
    ...
    if (!permanentDeleteResponse.ok) {
      throw new Error(
        `Failed to permanently delete note: ${permanentDeleteResponse.status}`
      );
    }
