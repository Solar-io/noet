...
const ModalsCollection = ({
  ...
  showFolderModal, showNotebookModal, showRenameFolderModal, showRenameNotebookModal,
  showTagModal, showChangePassword, showUserAdmin, showCreateUser, showDeleteUserConfirm,
  showUserSettings, showOtpSetup, showBackupCodes,
  
  // Modal setters
  setShowFolderModal, setShowNotebookModal, setShowRenameFolderModal, setShowRenameNotebookModal,
  setShowTagModal, setShowChangePassword, setShowUserAdmin, setShowCreateUser, setShowDeleteUserConfirm,
  setShowUserSettings, setShowOtpSetup, setShowBackupCodes,
  
  // Form values and setters
  newFolderName, setNewFolderName, newNotebookName, setNewNotebookName, newTagName, setNewTagName,
  renameFolderName, setRenameFolderName, renameNotebookName, setRenameNotebookName,
  currentPassword, setCurrentPassword, newPasswordChange, setNewPasswordChange,
  confirmPasswordChange, setConfirmPasswordChange, newUserName, setNewUserName,
  newUserEmail, setNewUserEmail, newUserPassword, setNewUserPassword,
  confirmNewUserPassword, setConfirmNewUserPassword, sessionTimeoutHours, setSessionTimeoutHours,
  
  // Editing states
  editingFolder, setEditingFolder, editingNotebook, setEditingNotebook,
  
  // Data
  users, setUsers, user, setUser, sessionTimeout, setSessionTimeout, otpEnabled, backupCodes,
  
  // Actions
  createFolder, createNotebook, createTag, renameFolder, renameNotebook, changePassword,
  createUser, deleteUser, exportUserData, setupOtp, disableOtp, logSecurityEvent
}) => {
  return (
    <>
      {/* Folder Modal */}
...
const NoetApp = () => {
  ...
  const handleLoginSuccess = (authenticatedUser) => {
    ...
    setSessionTimeout(authenticatedUser.sessionTimeout || 30);
    setSessionTimeoutHours(((authenticatedUser.sessionTimeout || 30) / 60).toString());
  ...
  const deleteNote = (noteId) => {
    if (confirm('Are you sure you want to delete this note?')) {
      ...
      if (noteToDelete) {
        ...
        saveUserData({ trashedNotes: updatedTrashedNotes });
      }
  ...
  const createFolder = () => {
    if (newFolderName.trim()) {
      ...
      saveUserData({ folders: updatedFolders });
      setNewFolderName('');
  ...
  const deleteFolder = (folderId) => {
    if (confirm('Delete this folder? Notebooks inside will be moved to the root level.')) {
      ...
      saveUserData({ notebooks: updatedNotebooks, folders: updatedFolders });
    }
  };
  ...
  const createNotebook = () => {
    if (newNotebookName.trim()) {
      ...
      setNotebooks(updatedNotebooks);
      saveUserData({ notebooks: updatedNotebooks });
      setNewNotebookName('');
  ...
  const deleteNotebook = (notebookId) => {
    if (confirm('Delete this notebook? Notes will be moved to Inbox.')) {
      ...
      setNotes(updatedNotes);
      saveUserData({ notebooks: updatedNotebooks, notes: updatedNotes });
    }
  };
  ...
  const deleteTag = (tagName) => {
    if (confirm('Delete this tag? It will be removed from all notes.')) {
      ...
      setNotes(updatedNotes);
      saveUserData({ tags: updatedTags, notes: updatedNotes });
    }
  };
  ...
  const handleNotebookReorderDrop = (e, targetFolderId, targetIndex) => {
    e.preventDefault();
    e.stopPropagation();
    if (!draggedNotebook) return;
    
    // Get notebooks in the target folder
    const targetNotebooks = notebooks.filter(n => n.folderId === targetFolderId);
    const otherNotebooks = notebooks.filter(n => n.folderId !== targetFolderId && n.id !== draggedNotebook.id);
    
    // Remove dragged notebook from its current position
    const filteredTargetNotebooks = targetNotebooks.filter(n => n.id !== draggedNotebook.id);
    
    // Insert at new position
    const updatedTargetNotebooks = [
      ...filteredTargetNotebooks.slice(0, targetIndex),
      { ...draggedNotebook, folderId: targetFolderId },
      ...filteredTargetNotebooks.slice(targetIndex)
    ];
    
    // Combine all notebooks
    const updatedNotebooks = [...otherNotebooks, ...updatedTargetNotebooks];
  ...
  return (
    <div className="h-screen flex bg-white">
      ...
      <Sidebar
        ...
        onShowNotebookModal={() => setShowNotebookModal(true)}
        ...
        onFilterByTag={filterNotesByTag}
        onEditFolder={(folder) => {
        ...
        draggedNotebook={draggedNotebook}
        draggedTag={draggedTag}
        dragOverNotebook={dragOverNotebook}
        ...
        onNotebookReorderDrop={handleNotebookReorderDrop}
      />
      ...
      <ModalsCollection
        ...
        showRenameNotebookModal={showRenameNotebookModal}
        ...
        setShowRenameNotebookModal={setShowRenameNotebookModal}
        setShowTagModal={setShowTagModal}
        setShowChangePassword={setShowChangePassword}
        setShowUserAdmin={setShowUserAdmin}
        setShowCreateUser={setShowCreateUser}
        setShowDeleteUserConfirm={setShowDeleteUserConfirm}
        setShowUserSettings={setShowUserSettings}
        setShowOtpSetup={setShowOtpSetup}
        ...
        renameNotebookName={renameNotebookName}
        setRenameNotebookName={setRenameNotebookName}
...
// Sidebar Component
const Sidebar = ({
  user, currentView, setCurrentView, selectedNotebook, setSelectedNotebook,
  notes, notebooks, folders, tags, trashedNotes, expandedFolders, setExpandedFolders,
  otpEnabled, onShowFolderModal, onShowNotebookModal, onShowTagModal,
  onShowUserSettings, onShowChangePassword, onShowUserAdmin, onLogout,
  onCreateNewNote, onFilterByTag, onEditFolder, onEditNotebook,
  onDeleteFolder, onDeleteNotebook, onDeleteTag, leftPanelWidth,
  draggedNote, draggedNotebook, draggedTag, dragOverNotebook,
  onNoteDragStart, onNotebookDragStart, onTagDragStart, onNotebookDrop,
  onFolderDrop, onTagDrop, onDragOver, onDragEnter, onDragLeave, onDragEnd,
  onNotebookReorderDrop
}) => {
  const [showUserMenu, setShowUserMenu] = useState(false);
          ...
          <div className="relative">
            ...
            {showUserMenu && (
              <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg border border-slate-200 z-50">
                <div className="py-1">
                  <button 
                    onClick={() => {onShowUserSettings(); setShowUserMenu(false);}}
                  ...
                  <button 
                    onClick={() => {onShowChangePassword(); setShowUserMenu(false);}}
                    className="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 flex items-center space-x-2"
                  >
                    <Key size={16} />
                  ...
                  {user?.isAdmin && (
                    <button 
                      onClick={() => {onShowUserAdmin(); setShowUserMenu(false);}}
                      className="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 flex items-center space-x-2"
                    >
                      <Users size={16} />
                  ...
                  <div key={`folder-${folder.id}`} className="mb-2">
                    ...
                    {expandedFolders.has(folder.id) && folderNotebooks[folder.id] && (
                      <div className="ml-6 mt-1">
                        {folderNotebooks[folder.id].map((notebook, index) => (
                          <div key={`notebook-${notebook.id}`} className="mb-1">
                            <div
                              draggable
                              onDragStart={(e) => onNotebookDragStart(e, notebook)}
                              onDragEnd={onDragEnd}
                              onClick={() => selectNotebook(notebook)}
                              onDrop={(e) => {
                                if (draggedNote) {
                                  onNotebookDrop(e, notebook.id);
                                } else if (draggedNotebook) {
                                  onNotebookReorderDrop(e, folder.id, index);
                                }
                              }}
                              onDragEnter={() => {
                                if (draggedNote) {
                                  onDragEnter(notebook.id);
                                }
                              }}
                    ...
                    <div
                      ...
                      onDrop={(e) => {
                        if (draggedNote) {
                          ...
                          onNotebookReorderDrop(e, null, index);
                        }
                      }}
                      onDragEnter={() => {
                        if (draggedNote) {
        ...
        <div className="px-4 py-2 border-t border-slate-200">
          ...
          {tags.map((tag, index) => (
            <div key={tag}>
              ...
              <div 
                draggable
                onDragStart={(e) => onTagDragStart(e, tag)}
                onDragEnd={onDragEnd}
...
const NotesList = ({
  ...
  onToggleNoteShortcut, onDeleteNote, onNoteDragStart, onDragEnd,
  middlePanelWidth, onCreateNewNote
}) => {
...
const VersionHistoryPanel = ({ 
  ...
  if (!showVersionHistory || !selectedNote) return null;
...
const NoteEditor = ({ 
  ...
  const calculateContentSignificance = (oldContent, newContent, oldTitle, newTitle) => {
    ...
    const charThreshold = 25;
    const wordThreshold = 5;
    const percentThreshold = 0.03;
  ...
  const shouldCreateVersion = (note, newTitle, newContent, forceCreate = false) => {
    ...
    if (isSignificant) return true;
    
    if (lastVersionTime) {
      const timeDiff = Date.now() - new Date(lastVersionTime).getTime();
      const twoMinutes = 2 * 60 * 1000;
      if (timeDiff < twoMinutes) return false;
    }
  ...
  useEffect(() => {
    if (selectedNote) {
      ...
      setLastSavedTitle(selectedNote.title || 'Untitled Note');
  ...
  return (
    <div className="flex-1 flex">
                  ...
                  <div className="absolute top-10 left-0 w-36 bg-white rounded-lg shadow-lg border border-slate-200 z-50">
                    <div className="py-2">
                      ...
                      {[1, 2, 3, 4, 5, 6].map(level => (
                        <button
                          key={level}
                          onClick={() => {execCommand('formatBlock', level); setShowHeaderPicker(false);}}
                          className="w-full text-left px-4 py-2 text-sm hover:bg-slate-100"
        ...
        <div className="flex-1 p-6 overflow-y-auto bg-white relative">
          <style>
            {`
              ...
              .note-editor .code-block-wrapper {
                user-select: text;
              }
              .note-editor .code-editor-area {
                user-select: text;
      ...
      <VersionHistoryPanel 
        ...
        onViewCurrentVersion={viewCurrentVersion}
