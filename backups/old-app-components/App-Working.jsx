...
import { 
  ...
  List, ListOrdered, Search, Star, Trash2, Folder, Tag, Settings
} from 'lucide-react';
...
const NoteEditor = ({ selectedNote, onNoteChange, onCreateNewNote }) => {
  ...
  const renderElement = useCallback((props) => {
    switch (props.element.type) {
      case 'bulleted-list':
        return <ul className="list-disc list-inside ml-4 my-2" {...props.attributes}>{props.children}</ul>;
      case 'numbered-list':
        return <ol className="list-decimal list-inside ml-4 my-2" {...props.attributes}>{props.children}</ol>;
      case 'list-item':
...
const Sidebar = ({ currentView, onViewChange, user, onLogout, notebooks, tags, noteCount }) => {
  return (
      ...
      <div className="flex-1 p-4 overflow-y-auto">
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-medium text-gray-700 uppercase tracking-wide">Notebooks</h3>
            <button className="text-gray-400 hover:text-gray-600" title="Add Notebook">
              <Plus size={14} />
            </button>
          </div>
          <div className="space-y-1">
            {notebooks && notebooks.length > 0 ? (
              notebooks.map(notebook => (
                <button
                  ...
                  className={`w-full text-left p-2 rounded-lg transition-colors flex items-center space-x-2 ${
                    currentView === `notebook-${notebook.id}` ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-100'
                  }`}
                >
                  <Folder size={16} />
              ...
              <p className="text-sm text-gray-500 italic px-2">No notebooks yet</p>
            )}
          </div>
        </div>
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-medium text-gray-700 uppercase tracking-wide">Tags</h3>
            <button className="text-gray-400 hover:text-gray-600" title="Manage Tags">
              <Tag size={14} />
            </button>
          </div>
          <div className="space-y-1">
            {tags && tags.length > 0 ? (
              tags.slice(0, 5).map(tag => (
                <button
                  ...
                  className={`w-full text-left p-2 rounded-lg transition-colors flex items-center space-x-2 ${
                    currentView === `tag-${tag.name}` ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-100'
                  }`}
                >
...
const NotesList = ({ notes, selectedNote, onSelectNote, onCreateNewNote, currentView, searchQuery, onSearchChange, onDeleteNote, onStarNote }) => {
        ...
        {sortedNotes.length === 0 ? (
          <div className="p-4 text-center text-gray-500">
            {searchQuery ? (
              ...
              <div>
                <p>No notes in {currentView === 'all' ? 'this view' : currentView}</p>
                {currentView === 'all' && (
                  <button 
                    onClick={onCreateNewNote}
...
const NoetApp = () => {
  ...
  const enhancedInitialValue = [
    ...
    },
    {
      type: 'bulleted-list',
      children: [
        { type: 'list-item', children: [{ text: 'Rich text editing with lists, formatting, and more' }] },
        { type: 'list-item', children: [{ text: 'Smart search and filtering' }] },
        { type: 'list-item', children: [{ text: 'Notebooks and tags for organization' }] },
        { type: 'list-item', children: [{ text: 'Starred notes and trash functionality' }] },
        { type: 'list-item', children: [{ text: 'Auto-save and real-time updates' }] },
      ],
    },
    {
      type: 'paragraph',
      children: [{ text: 'Try creating lists and pressing Enter twice to exit them!' }],
    },
  ];
    ...
    const demoNotes = [
      ...
      },
      {
        ...
        content: JSON.stringify([
          { type: 'paragraph', children: [{ text: 'Q1 Planning Meeting - January 15, 2025' }] },
          { type: 'bulleted-list', children: [
            { type: 'list-item', children: [{ text: 'Review budget allocation' }] },
            { type: 'list-item', children: [{ text: 'Set team goals and objectives' }] },
            { type: 'list-item', children: [{ text: 'Plan quarterly milestones' }] },
          ]},
        ]),
      ...
      },
      {
        ...
        title: 'Project Ideas',
        content: JSON.stringify([
          { type: 'paragraph', children: [{ text: 'Random ideas for future projects:' }] },
          { type: 'numbered-list', children: [
            { type: 'list-item', children: [{ text: 'AI-powered writing assistant' }] },
            { type: 'list-item', children: [{ text: 'Collaborative note-taking platform' }] },
            { type: 'list-item', children: [{ text: 'Mobile app for quick voice notes' }] },
          ]},
        ]),
