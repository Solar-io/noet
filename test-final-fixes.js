...
async function runFinalTest() {
  try {
    ...
    notebooks.forEach(nb => console.log(`   📚 ${nb.name} (${nb.id.substring(0, 8)}...)`));
    
    console.log(`✅ Tags: ${tags.length} found`);
    tags.forEach(tag => console.log(`   🏷️  ${tag.name} (${tag.id.substring(0, 8)}...)`));
    
    console.log(`✅ Folders: ${folders.length} found`);
    folders.forEach(folder => console.log(`   📁 ${folder.name} (${folder.id.substring(0, 8)}...)`));
    ...
    notes.forEach(note => {
      if (note.notebook) {
        ...
        if (notebook) {
          ...
          console.log(`   ❌ "${note.title}" → Notebook ${note.notebook} not found!`);
        }
      }
    });
    
    console.log(`📊 ${notesWithResolvableNotebooks}/${notesWithNotebooks} notes have resolvable notebook names`);
    ...
    console.log(`📊 ${notesWithResolvableTags}/${notesWithTags} notes have resolvable tag names`);
    ...
    if (tags.length >= 2) {
      ...
      console.log(`   ${response.ok ? '✅' : '❌'} Tag reordering API: ${response.status}`);
    }
    
    if (notebooks.length >= 2) {
      console.log("Testing notebook reordering...");
      const response = await fetch(`${BASE_URL}/api/${USER_ID}/notebooks/reorder`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      console.log(`   ${response.ok ? '✅' : '❌'} Notebook reordering API: ${response.status}`);
    }
    
    if (folders.length >= 2) {
      console.log("Testing folder reordering...");
      const response = await fetch(`${BASE_URL}/api/${USER_ID}/folders/reorder`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      console.log(`   ${response.ok ? '✅' : '❌'} Folder reordering API: ${response.status}`);
    }

    // Summary
    console.log("\n🎉 UI Fixes Test Results:");
    console.log("========================");
    console.log(`📚 Notebook names: ${notesWithResolvableNotebooks > 0 ? '✅ Will show actual names' : '❌ May show generic labels'}`);
    console.log(`🏷️  Tag filtering: ${tags.length > 0 ? '✅ Tag options available' : '❌ No tag filter options'}`);
    ...
    console.log("2. 📚 Check note list shows actual notebook names (not 'Notebook')");
    ...
    const allGood = notebooks.length > 0 && tags.length > 0 && folders.length > 0 && notesWithResolvableNotebooks > 0;
    console.log(`\n🎯 Overall Status: ${allGood ? '✅ All systems ready for testing!' : '⚠️  Some issues detected'}`);
