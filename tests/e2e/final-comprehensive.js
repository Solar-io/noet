...
async function finalComprehensiveTest() {
  ...
  try {
    ...
    const noteStillInAll = allNotesData.find(
      (n) => n.id === newNote.id && !n.deleted
    );
    ...
    const noteInTrash = trashNotes.find(
      (n) => n.id === newNote.id && n.deleted
    );
    ...
    const restoreResponse = await fetch(
      `${API_BASE}/api/${userId}/notes/${newNote.id}/restore`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      }
    ...
    const restoredNote = allNotesRestored.find(
      (n) => n.id === newNote.id && !n.deleted
    );
    ...
    const trashAfterRestore = await fetch(
      `${API_BASE}/api/${userId}/notes?deleted=true`
    );
    const trashNotesAfterRestore = await trashAfterRestore.json();
    const noteStillInTrash = trashNotesAfterRestore.find(
      (n) => n.id === newNote.id
    );
    ...
    const softDeleteAgain = await fetch(
      `${API_BASE}/api/${userId}/notes/${newNote.id}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
    ...
    console.log(
      "10. Performing PERMANENT DELETE (UI permanent delete button)..."
    );
    ...
    if (!permanentDeleteResponse.ok) {
      throw new Error(
        `Permanent delete failed: ${permanentDeleteResponse.status}`
      );
    }
    ...
    // Check trash view
    const finalTrashNotes = await fetch(
      `${API_BASE}/api/${userId}/notes?deleted=true`
    );
    const finalTrashNotesData = await finalTrashNotes.json();
    const noteInFinalTrash = finalTrashNotesData.find(
      (n) => n.id === newNote.id
    );
