/**
 * Test Script: Note Count Feature (Simplified)
 *
...
try {
  ...
  console.log(`✅ Working in project directory: ${process.cwd()}`);
} catch (error) {
...
async function testNoteCountFeature() {
  ...
  try {
    ...
    const notebooksResponse = await fetch(
      `${backendUrl}/api/${userId}/notebooks`
    );
    ...
    for (const notebook of notebooks) {
      ...
      console.log(
        `  📖 ${notebook.name}: noteCount=${notebook.noteCount} (${
          hasNoteCount ? "✅" : "❌"
        })`
      );
    }
    ...
    console.log(
      `✅ Created notebook: ${newNotebook.name} (ID: ${newNotebook.id})`
    );

    // Test 3: Verify new notebook shows up with count
    console.log("\n📋 Test 3: Verify new notebook appears with count...");
    const updatedNotebooksResponse = await fetch(
      `${backendUrl}/api/${userId}/notebooks`
    );
    const updatedNotebooks = await updatedNotebooksResponse.json();

    const testNotebook = updatedNotebooks.find(
      (nb) => nb.id === newNotebook.id
    );
    if (testNotebook) {
      console.log(
        `✅ Test notebook found with noteCount: ${testNotebook.noteCount}`
      );
    ...
    const noteResponse = await fetch(`${backendUrl}/api/${userId}/notes`, {
      ...
      body: JSON.stringify({
        ...
        content: "Testing note count feature", // Use string content instead of Slate object
        notebook: newNotebook.id, // Use 'notebook' instead of 'notebookId'
      }),
    });
    ...
    console.log("\n📋 Test 5: Verify notebook count increased...");
    const finalNotebooksResponse = await fetch(
      `${backendUrl}/api/${userId}/notebooks`
    );
    const finalNotebooks = await finalNotebooksResponse.json();

    const finalTestNotebook = finalNotebooks.find(
      (nb) => nb.id === newNotebook.id
    );
    ...
    const deleteNoteResponse = await fetch(
      `${backendUrl}/api/${userId}/notes/${newNote.id}`,
      {
        method: "DELETE",
      }
    );
    ...
    const deleteNotebookResponse = await fetch(
      `${backendUrl}/api/${userId}/notebooks/${newNotebook.id}`,
      {
        method: "DELETE",
      }
    );
