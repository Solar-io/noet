...
async function testBackendConnection() {
  try {
    ...
    if (window.configService) {
      ...
      console.log('⚠️ configService not available on window, trying manual discovery...');
  ...
  try {
    ...
    const createResponse = await fetch(`${backendUrl}/api/demo-user/notes`, {
      ...
      body: JSON.stringify({
        ...
        content: 'This note was created from browser console to test delete functionality',
        tags: ['console-test']
      })
    });
    ...
    const deleteResponse = await fetch(`${backendUrl}/api/demo-user/notes/${newNote.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ 
        metadata: { 
    ...
    const trashResponse = await fetch(`${backendUrl}/api/demo-user/notes?deleted=true`);
    ...
    // Clean up
    await fetch(`${backendUrl}/api/demo-user/notes/${newNote.id}`, { method: 'DELETE' });
