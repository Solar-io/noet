...
async function checkAppTipTapFixes() {
  ...
  try {
    ...
    // Check for save before switch logic
    const hasSaveBeforeSwitch = appContent.includes("await saveCurrentContent");
    const hasHandleNoteSelection = appContent.includes("handleNoteSelection");
    const hasExplicitSave = appContent.includes(
      "Before switching notes, save current content"
    );
    ...
    const textLossFixesPresent = hasSaveBeforeSwitch || hasExplicitSave;
